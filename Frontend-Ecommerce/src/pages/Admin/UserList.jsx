...
function UserList() {
  ...
  const deleteHandler =async(id) =>{
    if(window.confirm("Are you sure ?  you can't back after this ")){
      try {
        await deleteUser(id)
      } catch (error) {
  ...
  {users?.map(user => (
    <tr key={user._id}>
      ...
      <td className="px-4 py-2"> 
        {editableUserId === user._id ?(
            <div className="flex items-center">
              ...
              <button onChange={()=> updateHandler(user._id)} className="ml-2 bg-blue-500 text-white py-2 px-4 rounded-lg"> <FaCheck/></button>
            </div>
        ) :(
          <div className="flex items-center">
            <p>{user.email}</p>
      ...
      <td className="px-4 py-2">
          {!user.isAdmin && (
            <div className="flex">
              <button onClick={()=> deleteHandler(user._id)} className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">
                <FaTrash/>
              </button>
            </div>
          )}
